...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if ( !billingPhoneInput || typeof window.intlTelInput === 'undefined') return;

    const SPF_Woo_Billing_Options = {
        inputId: 'billing_phone',
        config: PCAFE_SPF_WOO_DATA.configuration == 'custom' ? 'custom' : 'global',
        initialCountry: PCAFE_SPF_WOO_DATA.defaultCountry != '' ? PCAFE_SPF_WOO_DATA.defaultCountry : 'us',
        geoIp: PCAFE_SPF_WOO_DATA.geoCountry == 'yes' ? 1 : 0,
        validation: PCAFE_SPF_WOO_DATA.frontendValidation == 'yes' ? 1 : 0
    };

    new PCAFE_SPF_WOOCOMMERCE(SPF_Woo_Billing_Options,pcafe_spf_global_setting);

    if( PCAFE_SPF_WOO_DATA.enableShipping == 'yes' ) {
        const SPF_Woo_Shipping_Options = {
        ...
        new PCAFE_SPF_WOOCOMMERCE(SPF_Woo_Shipping_Options,pcafe_spf_global_setting);
    }
});
